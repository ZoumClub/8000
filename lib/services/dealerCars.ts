...
export async function updateDealerCar(id: string, data: Partial<DealerCarFormData>) {
  try {
...
export async function deleteDealerCar(id: string) {
  try {
    const { error } = await supabase
      .from("dealer_cars")
      .delete()
    ...
    console.error("Error deleting dealer car:", error);
    return { success: false, error };
  }
}
